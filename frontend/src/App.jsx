...
function ProblemTracker({ token, setToken }) {
  ...
  const fetchProblems = async () => {
    try {
      const response = await fetch('http://127.0.0.1:5000/api/problems', { headers: { 'Authorization': `Bearer ${token}` } });
      const data = await response.json();
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      await fetch(`http://127.0.0.1:5000/api/problems/${id}`, {
  ...
  const deleteProblem = async (id) => {
    try {
      await fetch(`http://127.0.0.1:5000/api/problems/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
      fetchProblems();
    } catch (err) { console.error(err); }
  };
...
function HistoryDashboard({ token, setToken }) {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const response = await fetch('http://127.0.0.1:5000/api/history', { headers: { 'Authorization': `Bearer ${token}` } });
        const data = await response.json();
